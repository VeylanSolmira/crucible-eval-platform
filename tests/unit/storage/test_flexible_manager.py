...
class TestFlexibleStorageManager(unittest.TestCase):
    ...
    def test_create_evaluation(self):
        ...
        # Create evaluation
        result = self.manager.create_evaluation(eval_id, code)
    ...
    def test_update_evaluation_status(self):
        ...
        # Create
        self.manager.create_evaluation(eval_id, code)

        # Update to running
        result = self.manager.update_evaluation(eval_id, status="running")
        self.assertTrue(result)

        # Verify update
        data = self.manager.get_evaluation(eval_id)
        self.assertEqual(data["status"], "running")
        self.assertIn("started_at", data)

        # Update to completed
        result = self.manager.update_evaluation(eval_id, status="completed", output="2", exit_code=0)
        self.assertTrue(result)
    ...
    def test_fallback_on_primary_failure(self):
        ...
        result = manager.create_evaluation(eval_id, code)
        self.assertTrue(result)
    ...
    def test_large_output_handling(self):
        ...
        result = self.manager.update_evaluation(eval_id, status="completed", output=large_output)
        self.assertTrue(result)
    ...
    def test_event_tracking(self):
        ...
        self.manager.update_evaluation(eval_id, status="running")

        # Get all events
        events = self.manager.get_events(eval_id)
    ...
    def test_list_evaluations_with_filter(self):
        ...
        for i in range(5):
            ...
            self.manager.create_evaluation(eval_id, f"code_{i}")

            # Update some to completed
            if i % 2 == 0:
                self.manager.update_evaluation(eval_id, status="completed")

        # List all
        all_evals = self.manager.list_evaluations()
        self.assertGreaterEqual(len(all_evals), 5)

        # List only completed
        completed = self.manager.list_evaluations(status="completed")

        # Should have 3 completed (0, 2, 4)
        completed_test_evals = [e for e in completed if e["id"].startswith("list-test-")]
    ...
    def test_delete_evaluation(self):
        ...
        self.manager.create_evaluation(eval_id, code)
        self.manager.add_event(eval_id, "test", "Test event")
        ...
        # Delete
        result = self.manager.delete_evaluation(eval_id)
        self.assertTrue(result)
