...
def test_file_storage():
    ...
    config = StorageConfig(file_storage_path="./storage/evaluations", enable_caching=True)
...
def test_database_storage():
    ...
    config = StorageConfig(database_url=database_url, prefer_database=True, enable_caching=True)
    return FlexibleStorageManager.from_config(config)
...
def analyze_evaluations(storage):
    ...
    if evaluations:
        ...
        for event in events:
            print(
                f"  - {event.get('timestamp', 'N/A')}: {event.get('type')} - {event.get('message')}"
            )
...
def main():
    ...
    parser.add_argument(
        "--storage-type", choices=["file", "database"], default="file", help="Storage type to test"
    )
