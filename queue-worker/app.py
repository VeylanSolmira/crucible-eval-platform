...
class QueueWorker:
    ...
    def __init__(self):
        ...
        self.client = httpx.AsyncClient(
            headers={"X-API-Key": self.api_key} if self.api_key else {}, timeout=30.0
        )
    ...
    async def handle_completion_event(self, message):
        ...
        try:
            ...
            logger.info(
                f"[{datetime.now(timezone.utc).isoformat()}] Received {channel} event for {eval_id}"
            )

            # Mark the queue task as complete
            if channel == "evaluation:completed":
                await self.client.post(
                    f"{self.queue_url}/tasks/{eval_id}/complete", json={"output": data}
                )
    ...
    async def route_task(self, task: Dict):
        ...
        try:
            ...
            response = await self.client.post(
                f"{executor_url}/execute", json={"eval_id": eval_id, "code": code, "timeout": 30}
            )
            ...
            if response.status_code == 200:
                ...
                if result["status"] == "running":
                    ...
                    logger.info(
                        f"[{datetime.now(timezone.utc).isoformat()}] Task {eval_id} started running on {executor_url}"
                    )
                    # The queue task will be completed when the container finishes
                elif result["status"] == "completed":
                    await self.client.post(
                        f"{self.queue_url}/tasks/{eval_id}/complete", json={"output": result}
                    )
                    # Publish completion event for storage worker
                    await self._publish_event(
                        "evaluation:completed", {"eval_id": eval_id, "result": result}
                    )
                else:
                    await self.client.post(
                        f"{self.queue_url}/tasks/{eval_id}/fail",
                    ...
                    await self._publish_event(
                        "evaluation:failed",
                        {"eval_id": eval_id, "error": result.get("error", "Execution failed")},
                    )
            ...
            await self.client.post(f"{self.queue_url}/tasks/{eval_id}/fail", json={"error": str(e)})
    ...
    async def run(self):
        ...
        try:
            await asyncio.gather(self.pull_and_route_tasks(), self.listen_for_completion_events())
        except Exception as e:
...
async def health():
    if worker:
        ...
        return {"status": status["status"], "service": "queue-worker", "details": status}
