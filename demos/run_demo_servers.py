    ...
    def start(self):
        ...
        class APIRequestHandler(http.server.BaseHTTPRequestHandler):
            ...
            def _handle_request(self, method):
                ...
                request = APIRequest(
                    method=HTTPMethod[method], path=self.path, headers=dict(self.headers), body=body
                )
...
def main():
    ...
    # Create platform
    platform = QueuedEvaluationPlatform(engine=engine, queue=queue, monitor=monitor)
    ...
    frontend_config = FrontendConfig(
        port=8000, api_base_url="http://localhost:8001", enable_monitoring=True
    )

    frontend = create_frontend(frontend_type=FrontendType.SIMPLE_HTTP, config=frontend_config)
