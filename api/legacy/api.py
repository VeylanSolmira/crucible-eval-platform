...
from src.shared.base import TestableComponent
from src.core.core import TestableEvaluationPlatform
...
class APIService(TestableComponent):
    ...
    def __init__(self, platform: Optional[TestableEvaluationPlatform] = None):
        ...
        self.storage = None  # Optional storage service
    ...
    def get_evaluation_history(self, limit: int = 100, offset: int = 0) -> Dict[str, Any]:
        """
        ...
        Returns:
            Dictionary with evaluations list and metadata
        """
        if self.storage and hasattr(self.storage, 'list_evaluations'):
            try:
                evaluations = self.storage.list_evaluations(limit=limit, offset=offset)
                return {
...
class RESTfulAPIHandler:
    ...
    def _setup_routes(self):
        ...
        def eval_handler(request: APIRequest) -> APIResponse:
            """Main evaluation endpoint - all evaluations are async"""
            data = request.json()
        ...
        def evaluation_history(request: APIRequest) -> APIResponse:
            ...
            try:
                ...
                history = self.api.get_evaluation_history(limit=limit, offset=offset)
                return APIResponse.json(history)
            except Exception as e:
                return APIResponse.json({
                    'error': str(e),
                    'type': type(e).__name__
        ...
        # Register routes
        self.routes[(HTTPMethod.POST, '/eval')] = eval_handler
        ...
        self.routes[(HTTPMethod.GET, '/evaluations')] = evaluation_history
...
def create_api_service(platform: TestableEvaluationPlatform, storage: Optional[Any] = None) -> APIService:
    """
...
if __name__ == "__main__":
    ...
    from src.execution_engine.execution import SubprocessEngine
