...
@app.post("/tasks")
async def enqueue_task(request: EnqueueRequest, x_api_key: Optional[str] = Header(None)) -> Dict:
...
async def get_task_status(eval_id: str) -> TaskStatusResponse:
    ...
    return TaskStatusResponse(
        ...
        if isinstance(task["status"], EvaluationStatus)
        else task["status"],
...
async def clear_queue() -> Dict:
    ...
    return {"status": "cleared", "tasks_removed": count}
