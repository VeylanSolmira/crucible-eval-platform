"""
Executor Service - Creates isolated Kubernetes Jobs for code execution
Uses Kubernetes API for secure, isolated execution environments
"""
...
k8s_batch_v1: Optional[client.BatchV1Api] = None
k8s_core_v1: Optional[client.CoreV1Api] = None
k8s_namespace = os.getenv("KUBERNETES_NAMESPACE", "crucible")


@asynccontextmanager
async def lifespan(app: FastAPI):
    # Startup
    global redis_client, event_handler_task, k8s_batch_v1, k8s_core_v1

    logger.info(f"Starting up executor service {executor_id}")

    # Initialize Kubernetes client
    try:
        # Load in-cluster config if running in K8s, else local config
        try:
        ...
        # Test connection
        k8s_core_v1.list_namespace()
        logger.info("Connected to Kubernetes API successfully")
