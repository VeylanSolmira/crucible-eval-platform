...
async function analyzeWikiLinks() {
  ...
  allFiles.forEach(file => {
    const hasIncoming = Array.from(incomingLinks.values()).some(
      links => links.has(file.relative)
    );
...
4. Link to orphaned documents from related pages\n`;
  ...
  console.log(`   - Top missing term: ${Object.keys(termCounts)[0]} (${termCounts[Object.keys(termCounts)[0]]} occurrences)`);
}
