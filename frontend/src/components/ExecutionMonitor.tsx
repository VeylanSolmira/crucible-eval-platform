  ...
  const handleKill = async () => {
    ...
    try {
      ...
      if (errorMessage.includes('not running') || errorMessage.includes('404')) {
        console.info(
          `[INFO] Evaluation ${evalId} not running on any executor, updating status instead`
        )
        try {
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        <div className="text-center py-12">
          ...
          <p className="text-gray-500">
            Submit an evaluation or select one from the running list to view logs
          </p>
        </div>
      </div>
    )
      ...
      <div className="flex flex-col h-full p-6 bg-white overflow-y-auto min-w-0">
        <div className="flex justify-between items-center mb-4 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            {evalId && (
              <span
                className={`px-3 py-1 text-sm font-medium rounded-full ${statusBadge.bg} ${statusBadge.text}`}
              >
                {statusBadge.label}
              </span>
            )}
        ...
        {showKillConfirm && (
          <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg">
            <p className="text-sm text-gray-700 mb-3">
              Are you sure you want to kill evaluation {evalId}? This will terminate the execution
              immediately.
            </p>
        ...
        <div className="grid grid-cols-3 gap-4 mb-6 flex-shrink-0">
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <div className="flex items-end gap-2">
              <div className="text-2xl font-mono font-semibold text-gray-900">{cpuPercentage}%</div>
        ...
        {showDebugPanel && (
          <div className="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
            <h4 className="text-sm font-medium text-gray-900 mb-3">
              Debug Information & Admin Controls
            </h4>

            <div className="space-y-2 text-xs text-gray-700 mb-4">
              <div>
                <strong>Eval ID:</strong> {evalId}
              </div>
              <div>
                <strong>Status:</strong> {logs?.status || 'unknown'}
              </div>
              <div>
                <strong>Started:</strong> {logs?.started_at || 'N/A'}
              </div>
              <div>
                <strong>Last Update:</strong> {logs?.last_update || 'N/A'}
              </div>
              <div>
                <strong>Container ID:</strong> {logs?.container_id || 'N/A'}
              </div>
              <div>
              ...
              <div className="flex gap-4 items-start">
                <div className="flex gap-2 flex-shrink-0">
                  <button
                    ...
                    disabled={
                      updateStatusMutation.isPending || showConfirmDialog || showStatusDialog
                    }
                  >
                    Mark as Failed
                  </button>
                ...
                {showConfirmDialog && (
                  <div className="flex-1 p-3 bg-white rounded border border-red-200">
                    <p className="text-sm text-gray-700 mb-3">
                      Are you sure you want to mark this evaluation as failed? This action cannot be
                      undone.
                    </p>
                    <div className="flex gap-2">
                      <button
                          ...
                          void (async () => {
                            if (selectedStatus) {
                              try {
                                ...
                                setToast({
                                  message: `Evaluation marked as ${selectedStatus}`,
                                  type: 'success',
                                })
                              } catch (error) {
                                setToast({
